            ...
            override fun onConnectionStateChange(
                ...
                when (newState) {
                    BluetoothProfile.STATE_CONNECTED -> {
                        // Register device immediately regardless of bond state.
                        // The original createBond() path prevented unbonded centrals
                        // (Linux/PC, iOS, some Android devices) from ever entering
                        // bluetoothDevicesMap, causing updateCharacteristic to send
                        // nothing silently. Our characteristics carry no encryption
                        // requirement, so bonding is not needed for correct operation.
                        handler?.post {
